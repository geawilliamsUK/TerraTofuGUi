...
1. `definitions/providers/gcp.toml` — `hashicorp/google`, variables `project`, `region`,
   `zone`; `required_ancestor = "project"`.
2. A `project` container definition (`google_project` or `logical` when the project is
   pre-existing — the manual flag already covers the latter).
3. `[providers.gcp]` sections in each resource file. Expected shapes for the Phase 1 set:
   - `virtual_network` → `google_compute_network` (+ `auto_create_subnetworks = false`)
   - `subnet` → `google_compute_subnetwork` (`network` via relation, `region` via var)
   - `compute_instance` → `google_compute_instance` with nested `boot_disk.initialize_params`
     (`map` on image) and `network_interface` (subnet via relation), `service_account`
     nested block conditional on `iam_binding`
   - `object_storage` → `google_storage_bucket` (`versioning { enabled }` nested when field)
   - `iam_role` → `google_service_account` + `google_project_iam_member` (`map` on
     permissions to `roles/viewer` / `roles/editor`); **partial**: IAM binding scope note.

The only Rust change is the provider's line in `BUILTIN_PROVIDERS`. The multi-provider
export then produces `out/gcp/` automatically.

## Phase 4 — Contribution guide and tooling

- `CONTRIBUTING.md` walking through the "checklist for a new definition" in
  `MAPPING_FORMAT.md`, with a worked example (adding `security_group`).
- `ttg catalog` gains `--strict` (warn on abstract fields unused by any provider,
  outputs that reference missing attributes) and `--example <type>` that prints a
  starter TOML for a new type.
- CI: `cargo test` + export every `examples/*.ttg.json` for every provider and, on a
  runner that has `tofu`, run `tofu validate` on each output. Definition PRs are gated on
  this.
- A `schemas/` directory with a JSON Schema for the project file, generated from the
  Rust types, so external tools can validate `.ttg.json`.
- Provider schema cross-check: an optional dev tool that pulls the provider's published
  schema (`tofu providers schema -json`) and flags arguments in definitions that the
  provider does not know, catching typos and version drift early.
